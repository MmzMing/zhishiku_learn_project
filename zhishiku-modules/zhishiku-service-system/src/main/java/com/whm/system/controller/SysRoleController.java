...
public class SysRoleController {
    ...
    public TableDataInfo<SysRole> pageQuery(SysRole sysRole, PageQuery pageQuery) {
        return sysRoleService.pageQuery(sysRole, pageQuery);
    }
    ...
    public R<Boolean> deleteById(@PathVariable("id") long id) {
        return R.ok(sysRoleService.lambdaUpdate().eq(SysRole::getId, id).set(SysRole::getDeleted, 1).update());
    }
}